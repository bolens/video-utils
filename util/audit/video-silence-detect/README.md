# video-silence-detect

Detect silent intervals in the first audio track.

Mode: **read**. Operation: `silence-detect`.
...
```bash
bin/video-utils video-silence-detect --help
```
...
```text
usage: video-silence-detect [-h] [--config CONFIG] [--apply] [-n] [-j JOBS]
                            [-q] [-o OUTPUT] [--output-dir OUTPUT_DIR]
